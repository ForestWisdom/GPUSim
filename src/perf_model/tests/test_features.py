...
def test_tensor_core_global_bytes_treats_c_as_write_only_for_beta_zero() -> None:
    task = GemmTask(
        tile_m=128,
        tile_n=128,
        tile_k=32,
    ...
    assert features.bytes_a == 64.0
    assert features.bytes_b == 128.0
    assert features.bytes_c == 256.0
    assert features.bytes_global == 64.0 + 128.0 + 256.0
